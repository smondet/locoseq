...
(******************************************************************************)
(* 
 * Basic manipulation of MIDI "Data" (which is extracted by the MidiFile module)
 *
 * - many "int" values are in [0; 255] but OCaml stores chars as integer31
 * so there's no optimization to do here
 * - there remains some "int32" as I can't guarantee there's no overflow
 * File/track IDs and Length are 4 bytes values, even If IDs can be considered
 * as 4 characters strings ("MTrk", ...)
 *
 * *)
(******************************************************************************)
